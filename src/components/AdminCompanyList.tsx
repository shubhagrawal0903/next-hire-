  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {companies.map((company) => (
        <div key={company.id} className="bg-background rounded-lg border border-border p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div className="flex-1">
              <h3 className="text-lg sm:text-xl font-bold text-text-primary flex items-start sm:items-center gap-2 flex-wrap">
                <span className="break-words">{company.name}</span>
                <span className="text-xs font-normal px-2 py-0.5 bg-yellow-500/10 text-yellow-600 rounded-full border border-yellow-500/20 whitespace-nowrap">
                  Pending Review
                </span>
              </h3>
            ...
            <button
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-2 bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors text-sm shadow-sm"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 p-3 sm:p-4 bg-surface/50 rounded-lg border border-border">
            <div className="flex flex-col">
              <span className="text-xs text-text-muted uppercase tracking-wider font-semibold mb-1">Contact</span>
              <div className="flex items-start gap-2 text-sm font-medium text-text-primary break-all">
                <Mail className="w-3.5 h-3.5 text-primary flex-shrink-0 mt-0.5" /> 
                <span className="break-all">{company.contactEmail}</span>
              </div>
            </div>
            <div className="flex flex-col">
              <span className="text-xs text-text-muted uppercase tracking-wider font-semibold mb-1">Website</span>
              {company.website ? (
                <a href={company.website} target="_blank" rel="noopener noreferrer" className="flex items-start gap-2 text-sm font-medium text-primary hover:underline break-all">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
                ...
                <span className="break-all">{company.registrationNumber || "N/A"}</span>
              </div>
            </div>
            <div className="flex flex-col">
              ...
              <div className="flex items-start gap-2 text-sm font-medium text-text-primary">
                <Building className="w-3.5 h-3.5 text-text-muted flex-shrink-0 mt-0.5" /> 
                <span className="break-words">{company.industry || "N/A"} ({company.companyType || "N/A"})</span>
              </div>
            </div>
          </div>
